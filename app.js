...
const missions = [
  { title: '식후 15분 걷기', text: '식사 종료 후 30분 안에 15분 걷기를 완료해 보세요.', phase: 'Phase 1' },
  { title: '거꾸로 식사법', text: '채소 → 단백질 → 탄수화물 순서를 한 끼라도 실천해 보세요.', phase: 'Phase 1' },
  { title: '하체 근력 루틴', text: '스쿼트 또는 런지를 10분 이상 수행해 GLUT4 자극을 주세요.', phase: 'Phase 2' },
  { title: '수면 7시간', text: '오늘은 7시간 이상 수면 확보를 목표로 해보세요.', phase: 'Phase 3' }
];

const giSwaps = [
  ['백미', '현미'], ['식빵', '통밀빵'], ['시리얼', '오트밀'], ['떡', '삶은 고구마'], ['설탕음료', '무가당 차'], ['과자', '견과류']
];
...
function sortDesc(items, key='date') { return [...items].sort((a,b)=> `${b[key]}${b.time||''}`.localeCompare(`${a[key]}${a.time||''}`)); }
...
function setDefaults() {
  ...
  if (glucoseTime && !glucoseTime.value) glucoseTime.value = `${String(today.getHours()).padStart(2,'0')}:${String(today.getMinutes()).padStart(2,'0')}`;
}
function renderDashboard() {
  const score = getMetabolicScore();
  qs('metabolicScore').textContent = score;
  qs('rangeFill').style.width = `${score}%`;
  qs('statusLabel').textContent = score >= 75 ? '대사 유연성 구역' : score >= 50 ? '전환 구역' : '스파이크 경계 구역';
  qs('statusDot').style.background = score >= 75 ? '#22c55e' : score >= 50 ? '#f59e0b' : '#ef4444';
  ...
  qs('homeRiskFood').textContent = getRiskFoods()[0]?.food || '데이터 필요';
  qs('homeExerciseGap').textContent = `${getExerciseGap()}일`;
...
function renderRoadmapChart() {
  ...
  items.forEach((item, i) => {
    const x = p + i * stepX;
    const y = h - p - ((item.value - min) / (max - min)) * (h - p * 2);
    const color = item.value >= 140 ? '#ef4444' : item.value >= 110 ? '#f59e0b' : '#22c55e';
    spikes += `<circle cx="${x}" cy="${y}" r="5" fill="${color}" />`;
  });
  svg.innerHTML = `
    <rect x="0" y="0" width="360" height="180" rx="18" fill="transparent" />
    <rect x="0" y="0" width="360" height="90" fill="rgba(239,68,68,.06)" />
    <rect x="0" y="90" width="360" height="90" fill="rgba(34,197,94,.04)" />
    <polyline fill="none" stroke="#38bdf8" stroke-width="3" points="${points}" />
    ${spikes}
    <text x="12" y="24" fill="#fca5a5" font-size="12">Spike Zone</text>
    <text x="12" y="168" fill="#86efac" font-size="12">Reset Zone</text>`;
}
function renderDiagnosis() {
  ...
  qs('diagInsight').textContent = homa >= 2.5
    ? `HOMA-R ${homa.toFixed(2)}로 인슐린 저항성 관리가 필요한 편입니다. 식후 걷기와 하체 근력 루틴을 우선 강화하세요.`
    : `HOMA-R ${homa.toFixed(2)}로 비교적 양호하지만, 식후 스파이크와 수면 패턴을 같이 관리하면 더 좋습니다.`;
  qs('diagnosisList').innerHTML = sortDesc(state.diagnosis).map(item => {
    const h = calculateHoma(item.fasting, item.insulin);
    return `<div class="list-item"><div class="list-item-top"><strong>${item.date}</strong><small>HOMA-R ${h.toFixed(2)}</small></div><div>공복 ${item.fasting} · 인슐린 ${item.insulin} · HbA1c ${item.a1c}%</div><small>${item.note || ''}</small></div>`;
  }).join('');
}
function renderGlucose() {
  const source = recentWithinDays(state.glucose.filter(x=>x.type===currentType), currentRange).sort((a,b)=> `${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`));
  const svg = qs('glucoseChart');
  if (!source.length) { svg.innerHTML = ''; qs('glucoseSummaryText').textContent = '기록이 부족합니다.'; return; }
  ...
  const stepX = (w-p*2)/Math.max(1, source.length-1);
  const target = currentType==='fasting' ? 100 : currentType==='post1' ? 140 : 120;
  const targetY = h-p - ((target-min)/(max-min))*(h-p*2);
  const pts = source.map((item,i)=> {
    const x=p+i*stepX; const y=h-p - ((item.value-min)/(max-min))*(h-p*2); return [x,y,item];
  });
  svg.innerHTML = `
    <line x1="${p}" y1="${targetY}" x2="${w-p}" y2="${targetY}" stroke="#f59e0b" stroke-dasharray="6 6" />
    <polyline fill="none" stroke="#22c55e" stroke-width="3" points="${pts.map(p=>`${p[0]},${p[1]}`).join(' ')}" />
    ${pts.map(([x,y,item])=>`<circle cx="${x}" cy="${y}" r="5" fill="${item.value>target?'#ef4444':'#38bdf8'}" />`).join('')}
    <text x="${p}" y="20" fill="#93c5fd" font-size="12">${currentType==='fasting'?'공복':'식후'} ${currentRange}일 흐름</text>`;
  const average = avg(values);
  const maxVal = Math.max(...values);
  qs('glucoseSummaryText').textContent = `평균 ${average.toFixed(1)} mg/dL · 최고 ${maxVal} mg/dL · 기준선 ${target} mg/dL`;
  qs('riskFoodList').innerHTML = getRiskFoods().map(item => `<div class="list-item"><div class="list-item-top"><strong>${item.food}</strong><small>평균 ${item.avg.toFixed(1)} mg/dL</small></div><small>${item.count}회 기록</small></div>`).join('') || '<div class="list-item"><strong>아직 음식별 데이터가 부족합니다.</strong></div>';
}
...
function renderDiet() {
  qs('giSwapList').innerHTML = giSwaps.map(([from,to]) => `<div class="chip"><span>${from}</span><strong>→ ${to}</strong></div>`).join('');
  ...
  qs('dietList').innerHTML = latest.map(item => `<div class="list-item"><div class="list-item-top"><strong>${item.date} · ${item.mealType}</strong><small>${item.giSwap || '대안 없음'}</small></div><div>${item.reverseMeal?'거꾸로 식사 ':'- '}· ${item.chopsticksOnly?'젓가락 위주 ':'- '}</div><small>${item.note || ''}</small></div>`).join('');
}
...
function renderExercise() {
  ...
  qs('gapMessage').textContent = gap >= 2 ? '연속 2일 이상 운동 공백입니다. 오늘 하체 근력 또는 식후 걷기를 넣어주세요.' : '운동 공백 관리가 잘 되고 있습니다.';
  qs('exerciseList').innerHTML = sortDesc(state.exercise).map(item => `<div class="list-item"><div class="list-item-top"><strong>${item.date} · ${labelExercise(item.kind)}</strong><small>${item.minutes}분</small></div><div>${item.intensity} · ${item.goldenTime?'골든타임':'일반 시간'}</div><small>${item.note || ''}</small></div>`).join('');
}
function labelExercise(kind) {
  return ({ walking:'식후 걷기', squat:'스쿼트', lunge:'런지', cardio:'유산소' })[kind] || kind;
}
function renderHormone() {
  ...
  qs('avgSleep').textContent = recent.length ? `${sleepAvg.toFixed(1)}시간` : '-';
  ...
  if (stressAvg && stressAvg >= 3) insights.push('스트레스가 높은 편입니다. 10분 호흡 또는 명상 루틴을 고정해 두는 것이 좋습니다.');
  if (medAvg && medAvg < 10) insights.push('명상 루틴이 짧습니다. 최소 10분을 목표로 해보세요.');
  if (!insights.length) insights.push('수면과 스트레스 관리가 비교적 안정적입니다. 이 흐름을 유지하면 대사 회복에 유리합니다.');
...
function renderRoadmap() {
  ...
  qs('phaseGrid').innerHTML = roadmapPhases.map(phase => `<div class="card"><div class="section-title-row"><h3>${phase.phase}</h3><span class="pill">${phase.weeks}</span></div><div class="insight-card"><strong>${phase.goal}</strong><p>${phase.actions}</p></div></div>`).join('');
}
function renderAll() {
  renderDashboard();
  renderDiagnosis();
...
function bindNavigation() {
  document.querySelectorAll('[data-nav]').forEach(btn => btn.addEventListener('click', () => renderView(btn.dataset.nav)));
}
function bindForms() {
  qs('diagnosisForm').addEventListener('submit', e => {
    e.preventDefault();
    const fd = new FormData(e.target);
    state.diagnosis.unshift(Object.fromEntries(fd.entries()));
  ...
  qs('hormoneForm').addEventListener('submit', e => {
    ...
    const fd = new FormData(e.target);
    const entry = Object.fromEntries(fd.entries());
    entry.sleep = Number(entry.sleep); entry.stress = Number(entry.stress); entry.meditation = Number(entry.meditation);
    entry.sevenHours = fd.get('sevenHours') === 'on'; entry.caffeineCut = fd.get('caffeineCut') === 'on';
...
function bindChartControls() {
  document.querySelectorAll('.seg-btn').forEach(btn => btn.addEventListener('click', () => {
    ...
    btn.classList.add('active'); currentRange = Number(btn.dataset.range); renderGlucose();
  }));
  document.querySelectorAll('.type-btn').forEach(btn => btn.addEventListener('click', () => {
    document.querySelectorAll('.type-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active'); currentType = btn.dataset.type; renderGlucose();
  }));
}
function bindMission() {
  qs('missionCompleteBtn').addEventListener('click', () => { state.missionDoneToday = true; renderAll(); });
  qs('missionSkipBtn').addEventListener('click', () => { state.missionIndex = (state.missionIndex + 1) % missions.length; state.missionDoneToday = false; renderAll(); });
...
function bindTimer() {
  ...
  qs('timerStartBtn').addEventListener('click', () => {
    ...
    timerInterval = setInterval(() => {
      if (timerSeconds > 0) timerSeconds--;
        ...
        clearInterval(timerInterval);
        qs('timerGuide').textContent = `${timerStages[timerStageIndex].name} 단계가 끝났습니다.`;
      }
      updateTimerDisplay();
    }, 1000);
  });
  ...
  qs('timerResetBtn').addEventListener('click', () => {
    clearInterval(timerInterval); timerStageIndex = 0; timerSeconds = timerStages[0].seconds; updateTimerDisplay();
  });
}
function bindGoldenAlarm() {
  qs('goldenAlarmBtn').addEventListener('click', () => {
    ...
    const targetH = String(Math.floor((total % (24*60)) / 60)).padStart(2, '0');
    const targetM = String(total % 60).padStart(2, '0');
    qs('goldenAlarmStatus').textContent = '계산 완료';
    qs('goldenAlarmText').textContent = `추천 걷기 시작 시간은 ${targetH}:${targetM} 입니다. 최소 15분 가볍게 걸어보세요.`;
  });
}
function bindRoadmapWeek() {
  qs('weekRange').addEventListener('input', e => { state.week = Number(e.target.value); renderRoadmap(); save(); });
}
