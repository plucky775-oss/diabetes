...
## v9 업데이트: 85점 개선형
- 검사 기록은 일부 항목만 입력해도 저장되도록 수정했습니다.
- C-peptide, 중성지방, HDL, LDL, ALT, 요산 등 선택 입력 항목을 추가했습니다.
- 홈 화면에 위험 신호 카드와 안정 카드가 표시됩니다.
- 검사 탭에 공복혈당, HbA1c, 식후 2시간, HOMA-IR 기준표를 추가했습니다.
- 상담 챗봇 답변을 한 말풍선에 몰아넣지 않고 `핵심 답변 → 수치 해석 → 오늘 할 행동 → 다음 기록` 단계 카드로 나눠 보이게 했습니다.
- 답변 대기 중에는 로딩 말풍선이 보여 사용자가 혼란스럽지 않게 했습니다.
